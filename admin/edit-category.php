            ...
            if (isset($_POST['submit'])) {
                ...
                try {
                    if (!empty($posterFileName)) {
                        ...
                        $query = "UPDATE category SET name='$name', image='$posterFilePath' WHERE id='$id'";
                    } else {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['submit'])) {
            ...
            try {
                if (!empty($posterFileName)) {
                    // If a new image is uploaded, update both name and image
                    $query = "UPDATE subcategory SET name='$name', image='$posterFilePath' WHERE id='$id'";
                } else {
                    // If no new image is uploaded, update only the name
                    $query = "UPDATE subcategory SET name='$name' WHERE id='$id'";
                }
...
if(isset($_GET['sub-subcategoryId'])){
    $id=$_GET['sub-subcategoryId'];
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['submit'])) {
            // Function to sanitize input data
            function sanitizeInput($data) {
            ...
            try {
                if (!empty($posterFileName)) {
                    // If a new image is uploaded, update both name and image
                    $query = "UPDATE 2subcategory SET name='$name', image='$posterFilePath' WHERE id='$id'";
                } else {
                    // If no new image is uploaded, update only the name
                    $query = "UPDATE 2subcategory SET name='$name' WHERE id='$id'";
                }
    ...
    // Fetch current category data
    $query = "SELECT * FROM 2subcategory WHERE id='$id'";
    $data = mysqli_query($con, $query);
    while ($row = mysqli_fetch_assoc($data)) {
...
?>
       <div class="container-fluid pt-4 px-4">
        <div class="col-sm-12 ">
                    <div class="bg-light rounded h-100 p-4">
                        <h6 class="mb-4">Edit Second Sub Category</h6>
                        <form  method="POST" enctype="multipart/form-data">
                            <div class="row mb-3">
                                <label for="movie-title" class="col-sm-2 col-form-label">Category Name</label>
                                <div class="col-sm-10">
